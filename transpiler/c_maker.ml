...
let make_c_header = 
(** This function generates the header of the C code. *)
  "#include <stdio.h>\n" ^ 
  "#include <stdlib.h>\n" ^
  "#include <windows.h>\n" ^
...
(** This function generates the C code for the actors. *)
...
(** This function finds the functions and signals in the system graph. *)
  ...
  | IRSYFunction(name, attrl, signall, paraml)::tl ->
    ...
    | _ -> failwith "sy_find_functions_and_signals: IRSYFunction: inlinedCode expected"
    )
  | IRUnkownVertex(name, attrl, signall, paraml)::tl ->
...
(** This function generates the C code for the system graph. *)
  | IRSystemgraph(vl, el) ->
